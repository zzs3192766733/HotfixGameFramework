...
namespace UnityGameFramework.Runtime
{
    ...
    public sealed class UIForm : MonoBehaviour, IUIForm
    {
        ...
        public void OnInit(int serialId, string uiFormAssetName, IUIGroup uiGroup, bool pauseCoveredUIForm, bool isNewInstance, object userData)
        {
            ...
            try
            {
                m_UIFormLogic.ScriptGenerator();
            }
            catch (Exception exception)
            {
                Log.Error("UI form '[{0}]{1}' ScriptGenerator with exception '{2}'.", m_SerialId, m_UIFormAssetName, exception);
            }
            
            try
            {
                m_UIFormLogic.RegisterEvent();
            }
            catch (Exception exception)
            {
                Log.Error("UI form '[{0}]{1}' RegisterEvent with exception '{2}'.", m_SerialId, m_UIFormAssetName, exception);
            }
            
            try
            {
                m_UIFormLogic.OnCreate();
            }
            catch (Exception exception)
            {
                Log.Error("UI form '[{0}]{1}' OnCreate with exception '{2}'.", m_SerialId, m_UIFormAssetName, exception);
            }
        ...
        {
            for (int i = 0; i < m_UIFormLogic.UIWidgets.Count; i++)
            {
                UIWidget uiWidget = m_UIFormLogic.UIWidgets[i];
                if (uiWidget == null)
                {
                    continue;
                }
                uiWidget.RemoveAllUIEvent();
